...
export default function StudentProfile() {
  ...
  useEffect(() => {
    if (user) { api.getStudentAnalytics(user.user_id).then(setAnalytics).catch(console.error).finally(() => setLoading(false)); }
  }, [user]);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {user?.batch && (
          <View style={styles.badgeRow}>
            <View style={styles.badge}><Text style={styles.badgeText}>{user.batch}</Text></View>
            {user.exam_year && <View style={[styles.badge, { backgroundColor: theme.colors.accentLight + '30' }]}><Text style={[styles.badgeText, { color: theme.colors.accentDark }]}>Target: {user.exam_year}</Text></View>}
          </View>
        )}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Details</Text>
          <View style={styles.detailCard}>
            {[{ icon: 'school', label: 'Course', value: user?.course || 'Not set' }, { icon: 'book', label: 'Optional', value: user?.optional_subject || 'Not set' }, { icon: 'people', label: 'Mentor', value: user?.mentor?.name || 'Not assigned' }].map((item, idx) => (
        ...
        {analytics && (
          <View style={styles.section}>
            ...
            <View style={styles.perfGrid}>
              {[{ label: 'Overall', value: `${analytics.overall_pct}%`, color: theme.colors.primary }, { label: 'Done', value: analytics.completed, color: theme.colors.success }, { label: 'Hours', value: `${analytics.total_hours}h`, color: theme.colors.accent }, { label: 'Tasks', value: analytics.completed_tasks, color: theme.colors.primaryLight }, { label: 'Sessions', value: analytics.total_sessions, color: theme.colors.warning }, { label: 'Pending', value: analytics.pending_tasks, color: theme.colors.danger }].map((item, idx) => (
                <View key={idx} style={styles.perfCard}><Text style={[styles.perfValue, { color: item.color }]}>{item.value}</Text><Text style={styles.perfLabel}>{item.label}</Text></View>
              ))}
            </View>
          </View>
        )}
        <View style={styles.section}>
          <TouchableOpacity testID="logout-button" style={styles.logoutBtn} onPress={confirmLogout}><Ionicons name="log-out-outline" size={20} color={theme.colors.danger} /><Text style={styles.logoutText}>Logout</Text></TouchableOpacity>
        </View>
        <View style={{ height: 30 }} />
      </ScrollView>
      {showLogoutConfirm && (
        <View style={styles.modalOverlay}><View style={styles.modalCard}><Text style={styles.modalTitle}>Logout</Text><Text style={styles.modalBody}>Are you sure you want to logout?</Text>
